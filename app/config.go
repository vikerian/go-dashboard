...
type AppConfig struct {
	...
	WebUI struct {
		IP4    string `json:"web_ip4"`
		Name   string `json:"web_name"`
		Title  string `json:"web_title"`
		Access struct {
			Counter   uint64   `json:"access_counter"`
			BlockList []string `json:"access_blocklist,omitempty"`
			WhiteList []string `json:"access_whitelist,omitempty"`
		} `json:"access"`
	} `json:"webui"`
...
func ReadConfig(confFile string) (*AppConfig, error) {
